/* 
 * NeoEfx_TestBgColor- by Jerry Isdale
 *  NeoEffects is available at https://github.com/MauiJerry/NeoEffects_library
 *  this is a test tool to check out background color use
 */
...
void loop() {
  ...
  if (window.effectDone()) {
    windowState++;
    window.setBgColor(strip1.randomColor());
    ...
    switch (windowState) {
     ...
     case 9:
        ...
        effectCount = 20;
        window.setMultiSparkleEfx(GREEN, delayTime/2, delayTime, WINDOW_SIZE/5, effectCount);
        break;
    case 10:
